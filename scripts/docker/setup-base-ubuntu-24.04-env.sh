...
printf "\
deb [signed-by=/usr/share/keyrings/llvm-snapshot.gpg] http://apt.llvm.org/noble/ llvm-toolchain-$(lsb_release -cs)-17 main \n\
deb-src [signed-by=/usr/share/keyrings/llvm-snapshot.gpg] http://apt.llvm.org/noble/ llvm-toolchain-$(lsb_release -cs)-17 main\n\
deb [signed-by=/usr/share/keyrings/llvm-snapshot.gpg] http://apt.llvm.org/noble/ llvm-toolchain-$(lsb_release -cs)-18 main \n\
deb-src [signed-by=/usr/share/keyrings/llvm-snapshot.gpg] http://apt.llvm.org/noble/ llvm-toolchain-$(lsb_release -cs)-18 main\n\
deb [signed-by=/usr/share/keyrings/llvm-snapshot.gpg] http://apt.llvm.org/noble/ llvm-toolchain-$(lsb_release -cs)-19 main \n\
deb-src [signed-by=/usr/share/keyrings/llvm-snapshot.gpg] http://apt.llvm.org/noble/ llvm-toolchain-$(lsb_release -cs)-19 main\n" > /etc/apt/sources.list.d/llvm.list
...
# Confluent doesn't have support for Ubuntu 24.04 (noble). Once it's released change 'jammy' to '$(lsb_release -cs)'
...
deb [signed-by=/usr/share/keyrings/confluent.gpg] https://packages.confluent.io/clients/deb jammy main\n" \
...
gpg_retrieve_curl https://www.mongodb.org/static/pgp/server-8.0.asc mongodb
echo "deb [ arch=amd64,arm64 signed-by=/usr/share/keyrings/mongodb.gpg ] https://repo.mongodb.org/apt/ubuntu noble/mongodb-org/8.0 multiverse" \
...
if [ ! -f ubuntu-24.04.md ]; then
    echo "File 'ubuntu-24.04.md' not found!"
    exit 1
fi

PACKAGES_TO_INSTALL=$(cat ubuntu-24.04.md | tr '\n' ' ')
